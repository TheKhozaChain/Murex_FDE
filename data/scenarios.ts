...
export const incidents: Incident[] = [
  {
    ...
    description: "AUD-equivalent delta increased 18.4% across the Asia FX portfolio after the morning risk cycle.", impact: "AUD 12.8m gross exposure is affected; Daily Market Risk distribution is held pending validated recovery.", rootCause: "Stale USD/JPY closing rate in the APAC market-data set", confidence: 92, action: "After approval and source confirmation, simulate a bounded USD/JPY refresh, rerun the affected APAC controls, and release the report only after deterministic validation.", escalation: "Market Data Operations + Market Risk Control + Murex Production Support", guardrail: "The model cannot modify market data or declare success; only the allow-listed simulator and deterministic post-action controls can change incident state.",
    evidence: [
  ...
  {
    id: "HVB-2822", title: "Liquidity Datamart population shortfall", report: "Liquidity Coverage Pack", area: "Regulatory", severity: "Critical", status: "Escalated", businessDate: "03 Aug 2026", owner: "Regulatory Reporting", age: "4h 01m",
    description: "The Murex reporting chain stopped after ingesting 13 of 14 synthetic source segments, leaving the Liquidity Datamart population incomplete.", impact: "Regulatory pack may miss the internal sign-off window; distribution remains held.", rootCause: "Initially unconfirmed; additional evidence establishes upstream non-delivery of segment 14", confidence: 91, action: "After evidence expansion and approval, simulate a scoped segment re-ingestion and resume the Datamart chain, then validate every downstream control.", escalation: "Liquidity Data Services + Murex Production Support + Regulatory Reporting", guardrail: "Fail closed on the first pass; no remediation until current evidence supports a cause and a human approves the exact action scope.",
    evidence: [
