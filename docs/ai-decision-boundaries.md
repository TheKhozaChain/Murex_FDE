...
The workbench never changes real trades, market data, mappings, batch state, configuration or reports. Within the simulation, a human-approved request can invoke only the incident's named synthetic recovery; deterministic preconditions, post-action controls and audit remain authoritative. No recommendation can create a production capability or act on external infrastructure.
